...
ANGLE_MIN = 60           # 最小角度
ANGLE_MAX = 120          # 最大角度
EDGE_RATIO_MAX = 3       # 边长比例上限
EPSILON_FACTOR = 0.04    # 轮廓近似系数
...
def CenterGet(img, return_pts=False):
    ...
    for contour in contours:
        ...
        if len(approx) == 4:
            ...
            # 检查角度是否在合理范围内
            if all(ANGLE_MIN < angle < ANGLE_MAX for angle in angles):
                # 计算边长比例
                lengths = []
                for i in range(4):
                    x1, y1 = pts[i]
                ...
                # 边长比例限制
                if max_len / min_len <= EDGE_RATIO_MAX:
                    # 计算角度得分
                    ...
                    if total_score > best_score:
                        # 预计算中心点
                        temp_center = calculate_equidistant_center(pts)
                        if temp_center:
                            cx, cy = temp_center
                            # 核心识别：检查二值化图中中心点是否为白色(255)
                            # 如果中心是黑色的(0)，说明是水桶或实心黑块，则跳过
                            if frame[cy, cx] == 255:
